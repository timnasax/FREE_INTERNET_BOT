...
timoth({
  ...
  if (!reference) {
    return repondre("Please specify the book, chapter, and verse you want to read. Example: bible timoth 3:16", {
      contextInfo: {
        externalAdReply: {
          title: "Bible Reference Required",
          body: "Please provide a book, chapter, and verse.",
          thumbnailUrl: "https://files.catbox.moe/ehealu.jpg", // Replace with a suitable thumbnail URL
          sourceUrl: conf.GURL,
          mediaType: 1,
          showAdAttribution: true,
        },
...
╰─────────────────◆ `;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "TIMNASA-TMD HOLY BIBLE",
          body: `We're reading: ${data.reference}`,
          mediaType: 1,
          thumbnailUrl: "https://files.catbox.moe/ehealu.jpg", 
          sourceUrl: conf.GURL,
          showAdAttribution: true, 
        },
      },
...
timoth({
  ...
  if (!arg || arg.length === 0) {
    const replyText = "Example Usage: .code 255784xxxxxx.";
    return repondre(replyText);
  }

  try {
    // Notify user that pairing is in progress
    const replyText = "*Wait Timnasa Md is getting your pair code 🍼✅...*";
    await repondre(replyText);

    // Prepare the API request
    const encodedNumber = encodeURIComponent(arg.join(" "));
    const apiUrl = `https://timnasa-session-id-etyy.onrender.com/code?number=${encodedNumber}`;
...
*Timnasa Md Element Information:*
🚀 *Name:* ${data.name}
🚀 *Symbol:* ${data.symbol}
...
Regards ${conf.BOT} `;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "TIMNASA-TMD ELEMENT INFORMATION",
...
`;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "TIMNASA-TMD GITHUB USER INFO",
